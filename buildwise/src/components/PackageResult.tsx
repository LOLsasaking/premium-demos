        ...
        <>
          <div className="mt-6 overflow-hidden rounded-xl border border-line bg-[#050a12] shadow-glow">
            ...
            <div className={`grid min-h-[500px] ${viewMode === 'split' ? 'lg:grid-cols-2' : 'grid-cols-1'}`}>
              {viewMode !== '3d' && <div className={`relative overflow-auto border-line bg-black ${viewMode === 'split' ? 'lg:border-r' : ''}`}><div className="absolute left-2 top-1 z-10 font-mono text-[9px] uppercase tracking-wider text-cyan">{active?.name ?? '2D plan'} · coordinated</div><SheetEditor answers={answers} pkg={pkg} theme="autocad" sheetKind={active?.id ?? 'construction'} edits={edits} onChange={changeEdits} /></div>}
              {viewMode !== '2d' && <div className="relative min-h-[500px]"><Suspense fallback={<div className="grid h-[500px] place-items-center text-sm text-muted">Building synchronized model…</div>}><Plan3DViewer answers={answers} pkg={pkg} edits={edits} onChange={changeEdits} focusSheet={active?.id} className="h-[500px]" /></Suspense></div>}
            </div>
